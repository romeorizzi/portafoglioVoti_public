...
* score: 10.0
* date:  2021-06-22 11:23:46.130145
*/
/**
...
void sposta_intera_torre_v2(int n, int max, char peg_from, char peg_to, char peg_third) {
    ...
    if (curr_peg[max] != peg_to) {
        ...
        n = max;
        if(max > 0)
            sposta_intera_torre_v2(n, max, curr_peg[n], dest_peg[n], help_peg[n]);
    }
...
int main() {
    ...
    if (t == 0) {
        ...
        sposta_intera_torre_v2(N, N, curr_peg[N], dest_peg[N], help_peg[N]);
    }
