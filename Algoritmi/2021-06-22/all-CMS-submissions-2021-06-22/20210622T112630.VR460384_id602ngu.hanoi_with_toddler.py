...
* score: 90.0
* date:  2021-06-22 11:26:30.228372
"""
#!/usr/bin/env python3
# -*- coding: latin-1 -*-
...
def num_mosse_fun(num_dischi, piolo_from, piolo_to, level):
    ...
    if piolo_from != piolo_to:
        ...
        ll.remove(piolo_to)
        piolo_sup = ll[0]
        tot = num_mosse_fun_vera(num_dischi-1)
        tot += 1
        tot += num_mosse_fun(num_dischi-1, piolo_sup, d[level-1], level - 1)      
    else:    
        tot = num_mosse_fun(num_dischi-1, piolo_from, d[level-1], level - 1)
        
    return tot 

def num_mosse_fun_vera(num_dischi):
    ...
    if num_dischi == 1:   # ? devo per forza gestirlo come un caso base?
        mosse[num_dischi] = 1
        return 1
    mosse[num_dischi] = num_mosse_fun_vera(num_dischi-1) + 1 + num_mosse_fun_vera(num_dischi-1)
    return num_mosse_fun_vera(num_dischi-1) + 1 + num_mosse_fun_vera(num_dischi-1)    
...
if t == 0:
    print(num_mosse_fun(N, 'A', d[N], N)) # TO DO: stampa il numero di mosse
    # d[0] = None
else:
    sposta_torre(N, 'A', d[N], N)
