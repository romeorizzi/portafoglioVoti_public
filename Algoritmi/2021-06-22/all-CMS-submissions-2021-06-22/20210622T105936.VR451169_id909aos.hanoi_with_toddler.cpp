...
* score: 10.0
* date:  2021-06-22 10:59:36.286112
*/
/**
...
void sposta_intera_torre(int n, char peg_from, char peg_to, char peg_third) {
  ...
  if(n == 0)
    return;
   sposta_intera_torre(n - 1, curr_peg[n-1], third_peg(curr_peg[n-1], dest_peg[n-1]), dest_peg[n-1]);                  // TO BE DONE
   sposta_disco(n, curr_peg[n], dest_peg[n]);
   sposta_intera_torre(n - 1, peg_third, dest_peg[n-1], curr_peg[n-1]);
}
...
int main() {
  ...
  if(t==0)
    ...
    sposta_intera_torre(N, curr_peg[1], third_peg(curr_peg[1], dest_peg[1]), dest_peg[1]);// TO DO: fornisci la lista esplicita di mosse
  }
