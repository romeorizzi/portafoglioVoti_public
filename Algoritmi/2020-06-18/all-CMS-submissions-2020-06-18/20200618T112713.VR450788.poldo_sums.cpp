...
* task:  poldo_sums
* score: 8.0
* date:  2020-06-18 11:27:13.410616
*/
#include<iostream>
#include<vector>
#include<cassert>
...
int solve(int index_seq, int last_element, int index_sol){
    ...
    if(mat[index_seq][last_element] != 0)
        return mat[index_seq][last_element];
    ...
    //se devo ancora prendere il primo elemento
    if(last_element == -1){
        if(mat[index_seq+1][index_seq]!= 0)
            mat[index_seq+1][index_seq] = 1+ solve(index_seq+1, index_seq, index_sol+1);
        
        if(mat[index_seq+1][last_element]!= 0)
            mat[index_seq+1][last_element] = solve(index_seq+1, last_element, index_sol+1);
        mat[index_seq+1][last_element] = solve(index_seq+1, last_element ,index_sol);

        if(mat[index_seq+1][index_seq] > mat[index_seq+1][last_element]){
            seq[index_sol] = index_seq;
            return mat[index_seq+1][index_seq];
        }
        
        return mat[index_seq+1][last_element];
    }
    ...
    //se non posso prenderlo
    if(mat[index_seq+1][last_element])
        mat[index_seq+1][last_element] = solve(index_seq+1, last_element, index_sol);
    return mat[index_seq+1][last_element];
