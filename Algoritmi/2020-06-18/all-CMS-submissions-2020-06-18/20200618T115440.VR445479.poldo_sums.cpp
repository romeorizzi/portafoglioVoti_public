...
* task:  poldo_sums
* score: 35.0
* date:  2020-06-18 11:54:40.137116
*/
#include <iostream>
#include <fstream>
#include <string>
...
void sequences(int n, vector<int>* panino, vector<int>* gym, set<int>* max_sequence, set<int>* sequence, vector<set<int>>* succs){
    for(int s : (*succs)[n]){
        sequence->insert(s);
        sequences(s, panino, gym, max_sequence, sequence, succs);
    }
    if((*succs)[n].empty()){
...
int main() {
    ...
    for(n=0; n<N; n++){
        ...
        sequence.insert(n);
        sequences(n, &panino, &gym, &max_sequence, &sequence, &succs);
