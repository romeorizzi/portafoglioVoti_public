...
* task:  bin_heap_fill
* score: 5.0
* date:  2020-06-18 11:35:24.489185
*/
#include <iostream>
#include <cstdio>
#include <fstream>
...
int verify_heap_prop_f(int root, int bound){
    ...
    int temp;
    if(figlio_sx < bound && values[figlio_sx] < values[root] && f[figlio_dx] == 1){
        return 0; // viola la proprieta
    }else{
        if(figlio_sx < bound && values[figlio_sx] < values[root] && f[figlio_dx] == 0 && f[root] == 0){
    ...
    if(figlio_dx < bound && values[figlio_dx] < values[root] && f[figlio_dx] == 1){
        return 0; // viola la proprieta
    }else{
