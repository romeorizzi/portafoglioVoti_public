...
* task:  poldo_sums
* score: 27.0
* date:  2020-06-18 11:45:58.502938
*/
#include <iostream>
#include <fstream>
#include <string>
...
string solve ( int last, int index, int gymm){
    ...
    if( panini[index] < gymm +  panini[last]){
        ...
        pair_tmp = pair<int,string>(last*(size*10)+index , solve(last, index , gymm));
        ...
        if(count(s1.begin(), s1.end(), ' ') > count(s2.begin(), s2.end(), ' ') ){
            pair_tmp = pair<int,string>(last*(size*10)+index , s1 );
        }else{
            pair_tmp = pair<int,string>(last*(size*10)+index , s2 );
        }
        memo.insert(pair_tmp);
        return pair_tmp.second;
    }
