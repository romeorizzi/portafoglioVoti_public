...
* task:  poldo_sums
* score: 23.0
* date:  2020-06-18 10:27:08.063615
*/
#include <iostream>
#include <fstream>
#include <string>
...
void sequences(int n, vector<int>* panino, vector<int>* gym, set<int>* max_sequence, set<int>* sequence){
    set<int> succ = successors(n,panino,gym);
    for(int s : succ){
        sequence->insert(s);
        sequences(s, panino, gym, max_sequence, sequence);
    }
    if(succ.empty()){
...
int main() {
    ...
    for(n=0; n<N; n++){
        ...
        sequence.insert(n);
        sequences(n, &panino, &gym, &max_sequence, &sequence);
