...
* task:  poldo_sums
* score: 38.0
* date:  2020-06-18 13:28:49.259990
"""
#!/usr/bin/env/python
...
def mangia_panini(res, costo, pan):
    ...
    if len(res) > 0:
        ...
        if (len(res), res[len(res)-1]) not in memory.keys():
            ...
            if pan[0] >= costo:
                ...
                if len(si_mangio) > len(no_mangio):
                    memory[(len(res), res[len(res)-1])] = si_mangio
                    return si_mangio

                memory[(len(res), res[len(res)-1])] = no_mangio
                return no_mangio
        ...
        return memory[(len(res), res[len(res)-1])]
