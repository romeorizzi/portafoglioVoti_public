...
* task:  bin_heap_fill
* score: 24.0
* date:  2020-06-18 11:29:18.335408
"""
def swap(x, y):
    s = x
    x = y
...
def check_F(last_couple):
    ...
    for i in range(n): 
        if ((i not in last_couple or stop not in last_couple) and i != stop and i in idx_f 
            and (check_E_couple(stop, int((i-1)/2)) or i == 0) 
            and (check_E_couple(i, int((stop-1)/2)) or stop == 0)
            # Caso in cui si scambia il figlio con il padre (e vicerversa): 
            # devo controllare se la relazione è E_buona anche sui fratelli
            and (stop != int((i-1)/2)
                or ((i % 2 == 0 and check_E_couple(i-1, stop)) 
                or (i % 2 != 0 and check_E_couple(i+1, stop) or stop == 0)))
            and (i != int((stop-1)/2) 
                or ((stop % 2 == 0 and check_E_couple(stop-1, i) 
                or (stop % 2 != 0 and check_E_couple(stop+1, i)) or i == 0)))):
