...
* task:  bin_heap_fill
* score: 25.0
* date:  2020-06-18 13:15:25.275104
*/
#include <iostream>
#include <cstdio>
#include <fstream>
...
int verify_heap_prop_f(int root, int bound){
    ...
    if(figlio_sx < bound && values[figlio_sx] < values[root] && ( f[figlio_sx] == 1 || f[root] == 1)){
        ...
        if(figlio_sx < bound && values[figlio_sx] < values[root] && f[figlio_sx] == 0 && f[root] == 0){
            ...
            ret = verify_heap_prop_f(root, bound);
            if(ret == 1){
                return ret;
            }
            values[root] = values[figlio_sx];
            values[figlio_sx] = temp;
        }if(figlio_sx < bound && values[figlio_sx] < values[root] && 2 * figlio_sx + 1 < bound && values[figlio_sx] > values[2* figlio_sx + 1] && f[figlio_sx] == 0 && f[2*figlio_sx +1] ==0){
            // scambio figlio con nipote sx
            temp = values[ 2 * figlio_sx + 1];
            values[2 * figlio_sx + 1] = values[figlio_sx];
            values[figlio_sx] = values[2 * figlio_sx + 1];
            // li scambio e verifico dal padre
            ret = verify_heap_prop_f(root, bound);
            if(ret == 1){
                return ret;
            ...
            values[figlio_sx] = values[2 * figlio_sx + 1];
            values[2 * figlio_sx + 1] = temp;
        }
        if(figlio_sx < bound && values[figlio_sx] < values[root] && 2 *( figlio_sx + 1) < bound && values[figlio_sx] > values[2* (figlio_sx + 1)] && f[figlio_sx] == 0 && f[2*(figlio_sx +1)] ==0){
            // scambio figlio con nipote dx
            temp = values[ 2 * (figlio_sx + 1)];
            values[2 *( figlio_sx + 1)] = values[figlio_sx];
            values[figlio_sx] = 2 * (figlio_sx + 1);
            // li scambio e verifico dal padre
    ...
    if(figlio_dx < bound && values[figlio_dx] < values[root] && (f[figlio_dx] == 1 || f[root] == 1)){
        ...
        if(figlio_dx < bound && values[figlio_dx] < values[root] && f[figlio_dx] == 0 && f[root] == 0){
            ...
            values[root] = values[figlio_dx];
            values[figlio_dx] = temp;
        }
        if(figlio_dx < bound && values[figlio_dx] < values[root] && 2 * figlio_dx + 1 < bound && values[figlio_dx] > values[2* figlio_dx + 1] && f[figlio_dx] == 0 && f[2*figlio_dx +1] ==0){
            // scambio figlio con nipote sx
            temp = values[ 2 * figlio_dx + 1];
            values[2 * figlio_dx + 1] = values[figlio_dx];
            values[figlio_dx] = 2 * figlio_dx + 1;
            // li scambio e verifico dal padre
            ret = verify_heap_prop_f(root, bound);
            if(ret == 1){
                return ret;
            }
            values[figlio_dx] = values[2 * figlio_dx + 1];
            values[2 * figlio_dx + 1] = temp;
        }
        if(figlio_dx < bound && values[figlio_dx] < values[root] && 2 *( figlio_dx + 1) < bound && values[figlio_dx] > values[2* (figlio_dx + 1)] && f[figlio_dx] == 0 && f[2*(figlio_dx +1)] ==0){
            // scambio figlio con nipote dx
            temp = values[ 2 * (figlio_dx + 1)];
