...
* score: 0.0
* date:  2019-09-04 09:42:56.319450
*/
...
cella visita_internal(int i,int j,int superpoteri)
{
    ...
    if(ceckRight(i,j,superpoteri))
        cellaRight = visita(i,j+1,superpoteri);

    struct cella cellaDown;
    if(ceckDown(i,j,superpoteri))
        cellaDown = visita(i+1,j,superpoteri);

    struct cella cellaScelta;
    if(ceckRight(i,j,superpoteri) && ceckDown(i,j,superpoteri))
    {
        ...
        else
        {
            ...
            else
            {
                if(cellaRight.nPallini > cellaDown.nPallini)
                    cellaScelta = cellaRight;
                else if (cellaRight.nPallini < cellaDown.nPallini)
                    cellaScelta = cellaDown;
            }
        }
    }
    else if(ceckRight(i,j,superpoteri))cellaScelta = cellaRight;
    else if(ceckDown(i,j,superpoteri))cellaScelta = cellaDown;
    else
    {
        cellaScelta.vivo = 0;
