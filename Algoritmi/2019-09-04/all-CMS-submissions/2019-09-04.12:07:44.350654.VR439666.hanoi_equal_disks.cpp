...
* score: 0.0
* date:  2019-09-04 12:07:44.350654
*/
/**
...
int main() {
  ...
  do{
    if(B[topB]>=A[topA]){
      sposta_disco((int *)A[topA],A,B);
      topB++;
      topA--;
    }
    else if(B[topB]>=C[topC]){
      sposta_disco((int *)C[topC],C,B);
      topB++;
      topC--;
    }
    else if(C[topC]>=A[topA]){
      sposta_disco((int *)A[topA],A,C);
      topA--;
      topC++;
    }
    else if(A[topA]>=C[topC]){
      sposta_disco((int *)C[topC],C,A);
	topC--;
	topA++;
    }
      else if(A[topA]>=B[topB]){
	sposta_disco((int *)B[topB],B,A);
	topB--;
	topA++;
    }
      else if(C[topC]>=B[topB]){
	sposta_disco((int *)B[topB],B,C);
	topB++;
	topC--;
     }
  }
