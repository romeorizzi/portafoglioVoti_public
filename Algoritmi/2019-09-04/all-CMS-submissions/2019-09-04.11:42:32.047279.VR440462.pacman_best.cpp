...
* score: 0.0
* date:  2019-09-04 11:42:32.047279
*/
...
bool Pallino(int i, int j) {
    return posto[i][j] >=1 && posto[i][j]<=9;
}

bool DestraPossibile(int i, int j, int superpoteri) {
    ...
    if(destraValida && giuValida) { //possibili mosse vere, devo quindi scegliere 
        ...
        //valuto il  numero dei fantasmi!
            ...
            //VALUTO IL NUMERO DEI PALLINI TROVATI
                ...
                if(desta.pallini < giu.pallini) {
                    scelta = destra;
                }
                else if (desta.pallini < giu.pallini) {
                    scelta = giu;
                }
            }
...
int main() {
#ifdef EVAL
    assert( freopen("input.txt", "r", stdin) );Destra
