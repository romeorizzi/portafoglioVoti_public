...
* score: 0.0
* date:  2019-09-04 08:25:22.526584
*/
...
info visita(int i, int j, int superpoteri) {
    ...
    if(rightvalid && downvalid) {
        if(right info.vivo == 0 && downinfo.vivo == 1) {
            chos en = downinfo;
        } else i f(rightinfo.vivo == 1 && downinfo.vivo == 0) {
            chos en = rightinfo;
        } else { 
            if(r ightinfo.nfantasmi > downinfo.nfantasmi) {
                 chosen = rightinfo;
            } el se if(rightinfo.nfantasmi < downinfo.nfantasmi) {
                 chosen = downinfo;
            } el se {
