...
* score: 10.0
* date:  2019-09-04 09:12:38.461247
*/
...
int map(int m, int n, int maxM, int maxN, int pill){
    ...
    if(m + 1 > maxM && n + 1 <= maxN){
        switch(mappa[m][n + 1]){
            case '+':
                return map(m, n + 1, maxM, maxN, pill - dec);
                break;
            case '*':
                if(pill == 0)
                    return 0;
            ...
            case '8':
            case '9':
                return map(m, n + 1, maxM, maxN, pill + mappa[m][n + 1] - '0' - dec);
                break;
        }
    }
    ...
    if(m + 1 <= maxM && n + 1 > maxN){
        switch(mappa[m + 1][n]){
            case '+':
                return map(m + 1, n, maxM, maxN, pill - dec);
                break;
            case '*':
                if(pill == 0)
                    return 0;
            ...
            case '8':
            case '9':
                return map(m + 1, n, maxM, maxN, pill + mappa[m + 1][n] - '0' - dec);
                break;
        }
    }
    ...
    //posso andare sia in basso sia a destra
        ...
        if(mappa[m + 1][n] == '*' && mappa[m][n + 1] != '*')
            if(pillDx == 0)
                ...
                return map(m, n + 1, maxM, maxN, pillDx - dec);

        if(mappa[m + 1][n] != '*' && mappa[m][n + 1] == '*')
            if(pillBasso == 0)
                return 0;
            else   
                return map(m + 1, n, maxM, maxN, pillDx - dec);

        if(mappa[m + 1][n] == '*' && mappa[m][n + 1] == '*')
            if(pillBasso == 0 && pillDx == 0)
                return 0;
            else if(pillBasso != 0 && pillDx == 0)
                return map(m + 1, n, maxM, maxN, pillBasso - dec);
            else if(pillBasso == 0 && pillDx != 0)
                return map(m, n + 1, maxM, maxN, pillDx - dec);

        return map(m + 1, n, maxM, maxN, pillBasso) + map(m, n + 1, maxM, maxN, pillDx - dec);
    }
}
