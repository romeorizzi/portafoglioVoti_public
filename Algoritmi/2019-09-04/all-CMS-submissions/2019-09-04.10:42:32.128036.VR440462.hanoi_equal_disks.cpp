...
* score: 20.0
* date:  2019-09-04 10:42:32.128036
*/
/**
...
unsigned long long int conta_mosse(int diversi, int k[]) {//conta mosse
...
void sposta_disco(int n, char piolo_from, char piolo_to) { // stampa
...
int main() {
  ...
  for(int i=1; i<=N; i++) {
    ...
    if (check ==1) {//non trovo uguali
      diversi++;
      k[i]=1;
    }
    if(check == 0) {//trovo uguali
      k[i]= occ;
    }
    occ=1;
    check=1;
  ...
  if(t==0)
    printf("%llu\n", num_mosse=conta_mosse(diversi, k));
  else
