...
* score: 0.0
* date:  2020-07-14 09:08:37.800086
*/
#include <iostream>
#include <vector>
using namespace std;
    ...
    if(i == 0) {
        ...
        if(mat[gap][i][j] == -2) {
            if(a[a.size()-i] == b[b.size()-j]) {
                mat[gap][i][j] = max(costs[gap]+cost(a, b, i-1, j-1, 0, costs, mat), cost(a, b, i-1, j, 1, costs, mat));
            }
            else {
...
int main() {
    ...
    cout << cost(a, b, a.size(), b.size(), 0, costs, mat);
    return 0;
}