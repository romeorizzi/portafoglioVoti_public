...
* task:  risparmio
* score: 23.0
* date:  2020-07-14 07:56:19.373936
"""
#!/usr/bin/env python3
...
with open("input.txt", "r") as file:
    n, n_actions = file.readline().rstrip().split(" ")
    n = int(n)
    n_actions = int(n_actions)
    for i in range(n_actions):
        actions.append(file.readline().rstrip().split(" "))
...
with open("output.txt", "w") as file:
    ...
    for action in actions:
        if action[0] == '2':
            for up in update_lazy:
                update(up)
            update_lazy.clear()
            file.write(str(calcola_intervalli(int(action[1]), int(action[2]))))
            file.write("\n")
        if action[0] == '1':
            update_lazy.append(int(action[1]))
