...
* task:  risparmio
* score: 23.0
* date:  2020-07-14 09:21:42.975723
"""
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def operation(vin):
    ...
    if vin[0] == '1':
        update(1, 0, N-1, int(vin[1]))
    else:
        print(query(1, 0, N-1, int(vin[1]), int(vin[2]))[0])

def makeTree(n):
    return [[0,0,0] for _ in range(4*n)]

def query(cNode, lChild, rChild, start, end):
    global tree
    if start > end:
        # we don't want this node
        return 0
    if (start == lChild and end == rChild):
        # i already did the calculation in the father
        return tree[cNode]
    tm= (lChild + rChild)//2
    # find the children
    qLeft = query(cNode*2, lChild, tm, start, min(end, tm))
    qRight = query(cNode*2+1, tm+1, rChild, max(start, tm+1), end)
    if qLeft != 0:
        if qRight != 0:
            if qLeft[2] > 0 and qRight[1] > 0:
                return [qLeft[0] + qRight[0] - 1, qLeft[1], qRight[2]]
            else:
                return [qLeft[0] + qRight[0], qLeft[1], qRight[2]]
        else:
            return qLeft
    else:
        if qRight != 0:
...
def update(cNode, lChild, rChild, pos):
    if lChild == rChild:
        # we reached the destination
        # assuming it's a leaf, always true?
        tree[cNode] = [tree[cNode][0] ^ 1, tree[cNode][1] ^1, tree[cNode][2] ^ 1]
    else:
        tm = (lChild + rChild) // 2
        if (pos <= tm):
            update(cNode*2, lChild, tm, pos)
        else:
            update(cNode*2 + 1, tm+1, rChild, pos)
        if tree[cNode*2][2] > 0 and tree[cNode*2 +1][1] > 0:
            tree[cNode] = [tree[cNode*2][0] + tree[cNode*2 + 1][0] -1, tree[cNode*2][1], tree[cNode*2 + 1][2]]
        else:
            tree[cNode] = [tree[cNode*2][0] + tree[cNode*2 + 1][0], tree[cNode*2][1], tree[cNode*2 + 1][2]]
