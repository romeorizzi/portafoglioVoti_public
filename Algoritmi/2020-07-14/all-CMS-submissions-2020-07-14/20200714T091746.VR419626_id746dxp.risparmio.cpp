...
* task:  risparmio
* score: 1.0
* date:  2020-07-14 09:17:46.579288
*/
#include <algorithm>
#include <iostream>
#include <iterator>
...
int count_intervals2(int k, int j) {
    int count = 0, prev = inc[0];

    for (int i = k; i <= j; prev = inc[i], i++)
        if (inc[i] != prev)
            count++;

    return count == 0 && inc[0] == 1 ? count + 1 : count;
}
...
int main() {
    ...
    for (int k = 0, t, i, j; k < M; k++) {
        ...
        if (t == 2) {
            ...
            cout << endl;
#endif
            cout << count_intervals2(i, j) << endl;
        } else {
            flipBit(i);
#ifndef EVAL
            cout << "flip [" << i << "]" << endl;
