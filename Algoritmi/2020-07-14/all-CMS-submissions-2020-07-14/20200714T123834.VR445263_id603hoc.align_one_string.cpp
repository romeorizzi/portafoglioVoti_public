...
* score: 0.0
* date:  2020-07-14 12:38:34.964761
*/
// soluzione per align_one_string sfruttando programmazione dinamica con memoizzazione
#include <iostream>     
#include <algorithm>
#include <cassert>
#include <cstdio>
...
cell min_costo(int r, int c){ // aggiorno ricorsivamente la matrice di celle che contiene costo migliore e asterischi accumulati
    ...
    if(CELLS[r][c].costo==-1){ //aggiorno cella
        ...
        if(a[c] == b[r]){ // stesso carattere
            ...
            // }
            while(cell_t.n_ast > P){ // approccio greedy
                cell_t.costo += p[minP];
                cell_t.n_ast -= minP;
            }
...
int main() {
    ...
    if(CELLS[0][0].costo == INF) // non ho soluzione
        ...
        printf("%d", CELLS[0][0].costo + p[CELLS[0][0].n_ast] + 1);
