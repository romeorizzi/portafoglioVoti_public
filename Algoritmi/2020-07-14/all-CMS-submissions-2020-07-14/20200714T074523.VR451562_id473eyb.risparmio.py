...
* task:  risparmio
* score: 1.0
* date:  2020-07-14 07:45:23.212988
"""
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def query(start, end):
    ...
    while start < end:
        if start%2 == 1:
            lNode['best'] = tree[start]['best']
            lNode['sR'] = tree[start]['sR']
            lNode['sL'] = tree[start]['sL']
            lNode['ln'] = tree[start]['ln']
            start += 1
        if end%2 == 0:
            rNode['best'] = tree[end]['best']
            rNode['sR'] = tree[end]['sR']
            rNode['sL'] = tree[end]['sL']
            rNode['ln'] = tree[end]['ln']
            end -= 1
        start = start//2
        end = end//2
    if start > end:
