...
* score: 0.0
* date:  2020-07-14 09:54:13.559599
*/
...
cell min_costo(int r, int c){
    ...
    if(CELLS[i][j].costo==-1){ //aggiorno cella
        CELLS[i][j].costo= INF;
        cell cell_t;
        if(a[c] == b[c]){ // stesso carattere
            ...
            // }
            if(CELLS[i][j].costo > cell_t.costo + p[cell_t.n_ast]){ // consumo ast
                CELLS[i][j].costo = cell_t.costo + p[cell_t.n_ast];
                CELLS[i][j].n_ast = 0;
            }
            // CELLS[i][j].costo = min(CELLS[i][j].costo, cell_t.costo + p[cell_t.n_ast]);
        }
        // carattere diverso
        cell_t = min_costo(r,c+1);
        if(CELLS[i][j].costo > cell_t.costo + p[cell_t.n_ast+1]){ //aggiungo ast
                CELLS[i][j].costo = cell_t.costo;
                CELLS[i][j].n_ast = cell_t.n_ast+1;
        }
    }
    return CELLS[i][j];
}
