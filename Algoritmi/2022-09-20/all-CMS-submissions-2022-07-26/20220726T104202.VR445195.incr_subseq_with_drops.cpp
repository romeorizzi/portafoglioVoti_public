...
* score: 20.0
* date:  2022-07-26 10:42:02.851515
*/
#include <iostream>
...
void long_road_g1_kplus(int crolli_ammessi, int index, int length, int high){
	...
	for(int j=index;j<n+1;j++){
			...
			if(sequence[j]>high){
					...
					if(tail>(longest_sequence-length)){
						temp=j;
						long_road(crolli_ammessi,temp+1,temp2,high);
					}
					high=sequence[j];
				}
			else{
					if(crolli_ammessi>0 && tail>(longest_sequence-length)){
							length++;
							if(length>longest_sequence){
								longest_sequence=length;
							}
