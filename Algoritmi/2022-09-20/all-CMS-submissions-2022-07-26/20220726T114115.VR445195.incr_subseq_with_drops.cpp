...
* score: 10.0
* date:  2022-07-26 11:41:15.177604
*/
#include <iostream>
	...
	for(int j=index;j<n+1;j++){
			...
			if(sequence[j]>high){
					...
					high=sequence[j];
							...
							temp2=j+1;
							if(crolli_ammessi==0){
								long_road_zero_k(temp2,length, sequence[j]);
								}else if(crolli_ammessi>0){
...
void long_road_g1_k1( int index, int length, int high){
	...
	for(int j=index;j<n+1;j++){
			...
			if(sequence[j]>high){
					...
					temp=j;
					temp2=length;
					long_road_zero_k(temp+1,temp2+1,sequence[j]);
					}
			}
