...
* score: 0.0
* date:  2022-09-20 11:32:06.587440
*/
#include <cstdio>
#include <iostream>
#include <vector>
...
integer hasher(const vector<integer>& values) {
    integer val = 0xfabfcbafbcabcebc;
    for (integer h : values)
        val ^= val;
    return val;
}
...
vector<integer>& H, vector<int>& A, vector<int>& B, vector<int>& C) {
    ...
    // Altrimenti continuiamo la ricorsione nelle configurazioni con una torre più bassa
        ...
        for(int tower_to_lower = 0; tower_to_lower < config->size(); tower_to_lower++) {
            ...
            vector<integer>* new_config = new vector<integer>(*config);
    ...
    // Salva risultato
    mem.emplace(hasher(*config), result);
    return result;
}
...
integer costruisci(int N, int M, vector<integer>& H, vector<int>& A, vector<int>& B, vector<int>& C) {
    ...
    while(true) {
        ...
        unordered_map<integer, integer> mem;
