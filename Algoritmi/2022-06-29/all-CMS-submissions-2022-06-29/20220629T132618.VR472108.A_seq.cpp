...
* task:  A_seq
* score: 0.0
* date:  2022-06-29 13:26:18.415779
*/
/*
...
int find_length_decr(int index,int counter=0){
    ...
    for(int i=0;i<list_decrescente[vettore_recupero_d[i].column+1].size();i++){

        if(list_decrescente[vettore_recupero_d[i].column+1][i].prec==indice){
            printf("\nricorsione: %d %d\n",(list_decrescente[vettore_recupero_d[i].column+1][i]).index, counter+1 );
            tryMax = find_length_decr((list_decrescente[vettore_recupero_d[i].column+1][i]).index, counter+1 );
            if(tryMax>maximum){
                maximum=tryMax;
            }
        }
...
int build_list_together(int index){
    return find_length_decr(index);
    return find_length_cres(index);
...
int main(){
    ...
    for(int i=0;i< N_ELEMENTS;i++){
        printf("%d",build_list_together(i));
    }
