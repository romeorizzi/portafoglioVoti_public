...
* task:  A_seq
* score: 13.0
* date:  2022-06-29 12:10:28.435548
*/
#include <iostream>
...
int check(int sequenza[MAX_N+2], int index, int length,int start,bool down, int position[]){
	...
	lunghezza[start][position[start]]=tmp;
	if(lunghezza[start][position[start]]>best){
			best=lunghezza[start][position[start]];
		}
	position[start]++;
...
int main(){
	...
	int n_best=0;
	int posizioni[n]={0};
	check(sequence,0,1,0,false,posizioni);
