...
* task:  A_seq
* score: 0.0
* date:  2022-06-29 11:57:02.058123
"""
def findAseq1(lista):
    current_max = 0
    count_max = 0
    for i in range(0, len(lista)):
        current = 0
        if i == 0:
            current = findAseqRec(lista[1::], lista[i]+1, 1) - 1
        elif i == len(lista) - 1:
            current = findAseqRec(lista[0:len(lista)-1], lista[i]-1, 0)
        #elif lista[i] > lista[i-1] and lista[i] > lista[i+1]:
        else:
            current = findAseqRec(lista[0:i], lista[i]-1, 0) + findAseqRec(lista[i+1::], lista[i]+1, 1) - 1
        current += 1
...
def findAseqRec(lista, piv, mode):
    if mode == 0:
        ...
        if lista[len(lista)-1] < piv:
            ...
            return 0 + findAseqRec(lista[0:len(lista)-1], piv, mode)
    elif mode == 1:
        if len(lista) <= 0:
            return 0
        if lista[0] < piv:
            ...
            return 0 + findAseqRec(lista[1::], piv, mode)
