...
* task:  A_seq
* score: 48.0
* date:  2022-06-29 12:01:37.544664
*/
#include <iostream>
#include <fstream>
using namespace std;
...
int main(){
    ...
    if(goal == 2){
        ...
        for(int i=0; i<tot_len-1; i++)
            if(max_a == length_inc_norm[i]+length_dec_norm[tot_len-1-i]+1){

                int gigi = length_inc_norm[i];
                int gigio = length_dec_norm[tot_len-1-i];

                n_max_a++;
            }
        n_max_a /= 2;
        cout << n_max_a%1000000007 << endl;
    }
