#!/usr/bin/env python3
from sys import stderr, stdout, argv

def solve(n, H):
    max_left = [0] * n
    max_right = [0] * n
    A = [0] * n
    Q = 0
    print(max_left)
    print(max_right)
    print(A)
    print(Q)
    # Calcoliamo max_left (massima altezza a sinistra)
    max_left[0] = H[0]
    for i in range(1, n):
        max_left[i] = max(max_left[i-1], H[i])

    # Calcoliamo max_right (massima altezza a destra)
    max_right[n-1] = H[n-1]
    for i in range(n-2, -1, -1):
        max_right[i] = max(max_right[i+1], H[i])

    # Calcoliamo l'acqua intrappolata
    for i in range(n):
        A[i] = min(max_left[i], max_right[i])  # Minimo tra le due altezze
        if A[i] > H[i]:
            Q += A[i] - H[i]
    
    return Q, A
    
if __name__ == "__main__":
    debug_level = 0
    if len(argv) == 2:
        debug_level = int(argv[1])
    T = int(input())  # Numero di test case
    for t in range(1, 1 + T):
        if debug_level > 0:
            print(f"#\n# Testcase {t}:", file=stderr)
        n, _ = map(int, input().split())  # leggi n, ma non ci serve w (lo usiamo come H)
        H = list(map(int, input().split()))  # leggi le altezze H come lista
        if debug_level > 1:
            print(f"# {n=}\n# {H=}", file=stderr)
        Q, A = solve(n, H)  # Passiamo H come lista
        if debug_level > 2:
            print(f"# {Q=}, {A=}", file=stderr)
        print(Q)
        print(" ".join(map(str, A)))
