...
* task:  chordal_lexBFS
* score: 100.0
* date:  2020-09-18 07:42:25.496946
"""
#!/usr/bin/env python3
import fileinput
...
if goal==1:        
    ...
    for i in range(2,n+1):
        pos_i = 0
        pos_iprev = 0
        while pos_i < len(nei2[i]):
            if pos_iprev >= len(nei2[i-1]):
                print(i)
                exit(0)
            iprev_nei = nei2[i-1][pos_iprev]
            pos_iprev += 1
            if iprev_nei > nei2[i][pos_i]:
                print(i)
                exit(0)
            elif iprev_nei == nei2[i][pos_i]:
                pos_i += 1
