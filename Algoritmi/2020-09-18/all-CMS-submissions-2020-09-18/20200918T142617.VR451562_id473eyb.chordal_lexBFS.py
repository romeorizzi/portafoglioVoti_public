...
* score: 50.0
* date:  2020-09-18 14:26:17.954044
"""
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
class Chordal(object):
    ...
    def addOrd(self, node1, node2):
        """ Add connection between node1 and node2 """
        if node2>node1:
            if not(node1 in self._graphOrd):
                # if it's the first time we see this node
                self._graphOrd[node1] = [node2]
            else:
                if not(node2 in self._graphOrd[node1]):
                    # if node2 is not already in the connections of node1 
                    # self._graph[node1].add(node2)
                    self._graphOrd[node1].append(node2)

        
        # if undirected graph
        if not self._directed:
            if node1>node2:
                if not(node2 in self._graphOrd):
                    # if it's the first time we see node2
                    self._graphOrd[node2] = [node1]
                else:
                    if not(node1 in self._graphOrd[node2]):
                        # if node1 is not already in the connections of node1 
                        # self._graph[node2].add(node1)
                        self._graphOrd[node2].append(node1)
...
def task3(graph, n):
    ...
    for k in range(n, 0, -1): # run the elim order reversed
        ...
        # clq.add(k)
        # leftneigh.add(k)
        if k in graph._graphOrd:
            maxCl = max(maxCl, len(graph._graphOrd[k])+1)
    return maxCl
...
def task52(graph, n):
    ...
    for norder in range(n, 0, -1):
        # leftneigh = preproc & set(graph._graph[norder])
        used = set()
        if norder in graph._graphOrd:
            for preNode in graph._graphOrd[norder]: # left neighbors are the only ones with a color
                # i just have to eliminate their colors
                # available.remove(nc[preNode])
                used.add(nc[preNode])
        # preproc.add(norder)
