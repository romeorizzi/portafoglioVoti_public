...
* task:  altopiano
* score: 40.0
* date:  2020-09-18 14:18:43.364992
*/
#include <iostream>
#include <limits>
...
int main()
{
    ...
    */
    ...
    // compute query acceleretor table
    ...
    for (auto m = 0; m < M; ++m)
    {
        ...
        */
        ...
        if (op == 1)
        {
            ...
            auto x_range_offset = x % RANGE_SIZE;
            
            int max_a = LIMIT;
            for (auto j = 0; j < RANGE_SIZE + 2; ++j)
            {
                const auto k = x_range_index * RANGE_SIZE + j;
            ...
            if (x_range_offset < 2)
            {
                const auto r = x_range_offset - 1;
                int max_a = LIMIT;
                for (auto j = 0; j < RANGE_SIZE + 2; ++j)
                {
                    const auto k = r * RANGE_SIZE + j;
                    if (A[k] == A[k - 1] && A[k] == A[k - 2])
                        max_a = max(max_a, A[k]);
                }
                R[r] = max_a;
             ...
             // update right range
            if (x_range_index > (RANGE_SIZE - 2))
            {
                const auto r = x_range_index + 1;
                int max_a = LIMIT;
                for (auto j = 0; j < RANGE_SIZE + 2; ++j)
                {
                    const auto k = r * RANGE_SIZE + j;
        ...
        else
        {
            ...
            int max_a = LIMIT;
            const auto l_range = l / RANGE_SIZE + 1;
            const auto r_range = r / RANGE_SIZE - 1;
            
            // query inbetween subranges
            for (auto i = l_range; i <= r_range; ++i)
                max_a = max(max_a, R[i]);
                
            // query leftmost subrange
            const auto ll = l;
            const auto lr = min(r, l + RANGE_SIZE - (l % RANGE_SIZE));
            for (auto i = ll; i <= lr - 2 ; ++i)
            {
                if (A[i] == A[i + 1] && A[i] == A[i + 2])
                    max_a = max(max_a, A[i]);
            }
