...
* task:  altopiano
* score: 100.0
* date:  2020-09-18 13:31:59.655388
*/
#include <iostream>
#include <vector>
#include <queue>
...
const int minimo = numeric_limits<int>::min();
...
int massimo(vector<int>& tree, int id, int l, int r, int a, int b)
{
    ...
    return std::max(massimo(tree, 2 * id, l, meta, a, b), massimo(tree, 2 * id + 1, meta, r, a, b));
}
...
void aggiorna(vector<int>& tree, int id, int l, int r, int x, int y) {
    ...
    if(r > l + 1) {
        ...
        tree[id-1] = std::max(tree[2 * id - 1], tree[2 * id]);
    }
    else
        tree[id - 1] = y;
}
...
int main() {
    ...
    vector<int> altopiano(altezze, minimo);
        ...
        if(op == 1)
		{
            ...
            if(x - 2 >= 0)
			{
                if(cammino[x-2]==cammino[x-1] && cammino[x-1]==cammino[x])
                    ...
                    aggiorna(tree, 1, 0, potenza2_prossima(altezze), x-1, minimo);
            }
			...
			if(x - 1 >= 0 && x + 1 < altezze)
			{
                if(cammino[x - 1] == cammino[x] && cammino[x]==cammino[x + 1])
                    ...
                    aggiorna(tree, 1, 0, potenza2_prossima(altezze), x, minimo);
            }
