...
* task:  altopiano
* score: 10.0
* date:  2020-09-18 11:57:32.505044
*/
...
class node{
    public:
        ...
        void update(node s_l, node s_r){
            ...
            size = s_l.size + s_r.size;
        }

        string to_str(){
            string s = "{"+ to_string(alt) + "," + to_string(get<0>(l)) + "_" + to_string(get<1>(l)) + "," + to_string(get<0>(r)) + "_" + to_string(get<1>(r)) + ","+ to_string(has_alt) + ","+ to_string(is_leaf) +","+ to_string(size) + "}";
            return s;
        }
...
class seg_tree{
    public:
        ...
        string query(int l, int r){
            ...
            l += A.size() / 2;
            r += A.size() / 2;
            node s_l = node(-INF,false);
            node s_r = node(-INF,false);
            bool f_l = true;
            bool f_r = true;
            while(l < r){
                if(l %2 == 1){
...
void range(int b, int e, vector<int>& v){
    bool F = false;
    int MAX;
    for(int j = b+1;j<e;j++){
        int a = v[j-1];
