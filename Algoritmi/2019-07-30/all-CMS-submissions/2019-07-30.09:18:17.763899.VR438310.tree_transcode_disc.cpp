...
* score: 100.0
* date:  2019-07-30 09:18:17.763899
*/
/**
...
void two2one(int index)
{
  ...
  int n_childs = 0;
  //il primo offset è di 1 poiché è la distanza dal padre
  int prev_offset = 1;
  child_offsets[0] = 1; //la salvo anche negli offsets

  //Scorro i figli da destra verso sinistra, quindi decresco il loro indice
  while (child_index > index - node_offset)
  {
    //Salvo l'offset del figlio nell'array
    child_offsets[++n_childs] = prev_offset + in_tree[child_index];
    prev_offset += in_tree[child_index];
