...
* score: 0.0
* date:  2022-07-26 11:55:52.587073
"""
#!/usr/bin/env python3
import sys
...
def trova_lunghezza_massima(input_sequence, prev_item, allowed_jumps, local_indexes):
    ...
    if len(input_sequence) == 1:
        tempval = mem[local_indexes[0]][allowed_jumps]
        if tempval:
            return tempval
        mem[local_indexes[0]][allowed_jumps] = 1, 1
        return 1, 1
    ...
    for index, item in enumerate(input_sequence):
        ...
        #print(f"full input: {input_sequence} - {local_indexes}")
        ...
        if len(temp) >= max_len_sol:
            ...
            if mem[local_indexes[index]][jumps_left] != None:
                sol_len, num_max_sol_sub =  mem[local_indexes[0]][allowed_jumps]
            else:
            ...
            print(f"{index} {allowed_jumps} = {max_len_sol}, {num_max_sol}")
            mem[index][allowed_jumps] = (max_len_sol, num_max_sol)
