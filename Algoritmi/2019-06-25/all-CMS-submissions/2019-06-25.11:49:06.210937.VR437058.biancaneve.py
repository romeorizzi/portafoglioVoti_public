...
* task:  biancaneve
* score: 0.0
* date:  2019-06-25 11:49:06.210937
"""
def cambio(nani, pos_nani, p1, p2):
    temp = pos_nani[nani[p1 - 1] - 1]
    pos_nani[nani[p1 - 1] - 1] = pos_nani[nani[p2 - 1] - 1]
    pos_nani[nani[p2 - 1]- 1] = temp
    return pos_nani
...
def altezza(nani, pos_nani,h1, h2):
    hmin = min(h1,h2)
    hmax = max(h1,h2)
    i_min = min(pos_nani[nani[hmin-1]-1], pos_nani[nani[hmax-1]-1])
    i_max = max(pos_nani[nani[hmin-1]-1], pos_nani[nani[hmax-1]-1])
    for i in range(i_min, i_max + 1):
        if (nani[pos_nani[i]] < hmin or nani[pos_nani[i]] > hmax):
                return False
    return True
