...
* task:  is_shuffle_of
* score: 68.0
* date:  2019-06-25 11:25:48.870151
*/
#include <stdio.h>
#include <algorithm>
#include <assert.h>
    ...
    for (int i=0; i<N+M; i++){
        ...
        if (deck1[m] != deck2[n]){
            ...
            if (m < M && deck_shuffle[i] == deck1[m]){
                ...
                // printf("\tAssurdo: i=%d d1:%d d2=%d ds=%d\n", i, deck1[m], deck2[n], deck_shuffle[i]);
                // non avevo una scelta precedente
                if (last_choice == 0){
                    fprintf(fout, "0\n");
                    return 0;
