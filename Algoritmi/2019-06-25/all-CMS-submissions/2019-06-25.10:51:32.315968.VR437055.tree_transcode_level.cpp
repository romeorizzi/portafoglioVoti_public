...
* score: 90.0
* date:  2019-06-25 10:51:32.315968
*/
#include <stdio.h>
#include <algorithm>
#include <assert.h>
...
int main(){
    ...
    assert(1 == fscanf(fin, "%d", &codifica));

    if (codifica == 1){
        // TRASFORMO IN CODIFICA 2
        int root;
        assert(1 == fscanf(fin, "%d", &root));
        fprintf(fout, "%d", 2);
        ...
        int node = 1;
        int max_depth = 0;
        assert(1 == fscanf(fin, "%d", &ARR[0]));
        int i=1;
        while (node != 0){
            assert(1 == fscanf(fin, "%d", &node));
            ARR[i] = node;
            i++;
            if (node > max_depth)
                max_depth = node;
