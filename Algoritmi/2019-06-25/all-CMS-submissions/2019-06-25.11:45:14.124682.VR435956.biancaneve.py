...
* task:  biancaneve
* score: 0.0
* date:  2019-06-25 11:45:14.124682
"""
...
def check_con(dwarfs, h1, h2):
    start = 0
    for i in range(0, len(dwarfs)):
        if(int(dwarfs[i]) == h1):
            start = i
            break
    diff = h2 - h1
    counter = 0
    initial_pos = start - diff

    while (counter < diff + 1):
        if(int(dwarfs[initial_pos]) not in range (h1, h2+1) and counter == 0):
            initial_pos += 1
        elif(int(dwarfs[initial_pos]) not in range (h1, h2+1) and counter != 0):
            return False
        elif(int(dwarfs[initial_pos]) in range (h1, h2+1) and counter != 0):
            counter += 1
            initial_pos += 1
